...
@RestController
class MapController {
	static final Object API_KEY = "AIzaSyDnFTqZrD3xPsYmhXGXS5afz7isxUcOFPI";
...
//		System.out.println(location);
		...
		UriComponents uriB = UriComponentsBuilder.newInstance()
				.scheme("https")
				.host("maps.googleapis.com")
				.path("/maps/api/distancematrix/json")
				.queryParam("key", API_KEY)
				...
				.build();
		ResponseEntity<ResponseRoute> responseB = new RestTemplate().getForEntity(uriB.toUriString(), ResponseRoute.class);
		ResponseRoute bodyB = responseB.getBody();
